...
export default class WxQyLogin extends Component {
    ...
    jsonp() {
        const {url,app_key,is_get_user_info}=this.props;
        let fullUrl = url + 'wx-base/login/json-p-get-my-info2?' +
            '&key=' + app_key +
            '&is_user_info=' + is_get_user_info +
            '&url=' + encodeURIComponent(Base64.encode(location.href));
        const promise = Q.defer();
        JsonP(fullUrl, 'WxUserInfo' + Math.floor(Math.random() * 10000));
        const timed = setTimeout(function () {
            alert('登录超时！');
        }, 10000);
        window.WxUserInfo = function (res) {
            clearTimeout(timed);
